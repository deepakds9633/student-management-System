...
public interface MarkRepository extends JpaRepository<Mark, Long> {
    ...
    @org.springframework.data.jpa.repository.Query("SELECT s.name, s.course, AVG(m.marksObtained), AVG(m.maxMarks) FROM Mark m JOIN m.student s GROUP BY s.id, s.name, s.course ORDER BY AVG(m.marksObtained) DESC")
    List<Object[]> findTopPerformers();

    @org.springframework.data.jpa.repository.Query("SELECT s.name, s.course, AVG(m.marksObtained), AVG(m.maxMarks) FROM Mark m JOIN m.student s GROUP BY s.id, s.name, s.course ORDER BY AVG(m.marksObtained) ASC")
